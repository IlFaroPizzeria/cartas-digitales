(function(){
  ...
  var defaultMsg = 'Hola, me gustaría más información sobre la carta digital de Cartoca para mi restaurante.';
  ['nav-wa','hero-wa','final-wa','foot-wa','founder-wa'].forEach(function(id){
    var el = document.getElementById(id);
    if (el) el.href = waLink(defaultMsg);
  });

  document.getElementById('foot-year').textContent = '© ' + new Date().getFullYear() + ' Cartoca';

  // Mobile nav toggle
  var navToggle = document.getElementById('navToggle');
  if (navToggle) {
    navToggle.addEventListener('click', function(){
      landingEl.classList.toggle('nav-open');
    });
    document.querySelectorAll('.nav-mobile a').forEach(function(a){
      a.addEventListener('click', function(){ landingEl.classList.remove('nav-open'); });
    });
  }

  // Demo tabs (text panels + animated tap stages)
  ...
  // Calculator — real Cartoca pricing
  var BASE_PRICE = 50;
  ...
  function bump(){
    ...
    setTimeout(function(){ totalAmount.classList.remove('bump'); }, 220);
  }
  ...
  document.getElementById('menuMinus').addEventListener('click', function(){ state.menu = Math.max(0, state.menu - 1); render(true); });
  document.getElementById('menuPlus').addEventListener('click', function(){ state.menu = Math.min(80, state.menu + 1); render(true); });
  document.getElementById('reviewMinus').addEventListener('click', function(){ state.reviews = Math.max(0, state.reviews - 1); render(true); });
  document.getElementById('reviewPlus').addEventListener('click', function(){ state.reviews = Math.min(30, state.reviews + 1); render(true); });
  maintToggle.addEventListener('click', function(){ state.maint = !state.maint; render(true); });
