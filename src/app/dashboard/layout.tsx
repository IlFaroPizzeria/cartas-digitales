...
export default async function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  if (!negocio) {
    const nombrePendiente = user.user_metadata?.negocio_nombre as string | undefined
    if (nombrePendiente) {
      ...
      {negocio && !negocio.activo && (
        <div className="bg-amber-50 border-b border-amber-200">
          <div className="max-w-3xl mx-auto px-4 py-2.5 text-sm text-amber-800">
            Tu carta está pendiente de aprobación. Puedes configurarla mientras tanto — te
            avisaremos en cuanto esté activa y visible al público.
          </div>
        </div>
      )}
