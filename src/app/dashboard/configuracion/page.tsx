...
export default async function ConfiguracionPage() {
  ...
  const { data: negocio } = await supabase
    .from('negocios')
    .select(
      'nombre, tagline, telefono, email, direccion, color_fondo, color_header, color_acento, logo_url, idiomas_activos, idiomas_permitidos, plantilla, mostrar_barra_categorias'
    )
    .eq('owner_id', user.id)
    .maybeSingle()
